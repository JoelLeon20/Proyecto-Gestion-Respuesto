...
if (isset($_SERVER['HTTP_REFERER'])) {
    ...
    if (strpos($referer, 'usuario=jefe') !== false) {
        $expected_role = 'jefe_taller';
    } elseif (strpos($referer, 'usuario=mecanico') !== false) {
        $expected_role = 'mecanico';
    } elseif (strpos($referer, 'usuario=proveedor') !== false) {
...
try {
    ...
    if ($fila) {
        if ($expected_role && $fila['rol'] !== $expected_role) {
            // User is trying to login with wrong credentials for this specific login form
            $usuario_param = '';
            ...
            $redirect_url = "login.php?usuario=" . $usuario_param . "&error=4";
            if ($intended_page) {
        ...
        if ($expected_role) {
            ...
            $redirect_url .= "&usuario=" . $usuario_param;
        }
        header("Location: " . $redirect_url);
        exit;
    }
