...
if (!isset($_SESSION['usuario']) || $_SESSION['rol'] !== 'mecanico') {
    header('Location: login.php?usuario=mecanico');
    exit;
}
...
<head>
  ...
  <style>
    ...
    main.contenedor {
      background: #d9d9d9;
      /* reduced top padding from 10px to 5px to move cards even higher up like jefe de taller */
    ...
    .dashboard {
      ...
      max-width: 1200px;
      margin: 0 auto;
      justify-content: center; /* Added justify-content to center the cards */
    }
    .card {
      background: #fff;
